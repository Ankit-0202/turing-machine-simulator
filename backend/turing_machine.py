...
class TuringMachine:
    ...
    def parse_transitions(
        ...
        for idx, t in enumerate(transitions):
            ...
            if not all(
                ...
                raise ValueError(
                    f"Transition at index {idx} is missing required fields."
                )

            # Validate symbols: cannot contain ';' or whitespace
            invalid_symbols = {";"}
            if read_symbol != "*" and any(
                char in invalid_symbols or char.isspace() for char in read_symbol
            ):
                raise ValueError(
                    f"Invalid read_symbol '{read_symbol}' in transition at index {idx}."
                )

            if write_symbol != "*" and any(
                char in invalid_symbols or char.isspace() for char in write_symbol
            ):
                raise ValueError(
                    f"Invalid write_symbol '{write_symbol}' in transition at index {idx}."
                )

            # Validate state names: cannot contain ';' or whitespace
            if any(char in {" ", ";"} for char in current_state):
                raise ValueError(
                    f"Invalid current_state '{current_state}' in transition at index {idx}."
                )

            if any(char in {" ", ";"} for char in new_state):
                raise ValueError(
                    f"Invalid new_state '{new_state}' in transition at index {idx}."
                )

            # Validate direction
            if direction not in {"l", "r", "*"}:
                raise ValueError(
                    f"Invalid direction '{direction}' in transition at index {idx}."
                )
            ...
            if key in seen_keys and read_symbol != "*":
                raise ValueError(
                    f"Duplicate transition for state-symbol pair: ({current_state}, {read_symbol}) at index {idx}."
                )
            seen_keys.add(key)
            ...
            actual_new_state = (
                new_state if new_state != "*" else current_state
            )  # Assuming 'no change' means stay in current state
            ...
            logger.debug(
                f"Parsed transition {idx}: {current_state} {read_symbol} {write_symbol} {direction} {new_state}"
            )
    ...
    def parse_input(self, input_string: str) -> (List[str], int):
        ...
        for idx, char in enumerate(input_string):
            if char == "*" and not found_head:
                ...
                found_head = True
                logger.debug(
                    f"Found head indicator at position {idx}, setting head to {head}"
                )
            else:
                tape.append(char)
        ...
        if not found_head:
            head = 0
            logger.debug(
                "No head indicator '*' found in input string; defaulting head to position 0"
            )
    ...
    def step(self) -> Optional[Dict]:
        ...
        logger.debug(
            f"Step {self.steps + 1}: Current state: {self.current_state}, Read symbol: {current_symbol}, Head position: {self.head}"
        )
        ...
        logger.debug(
            f"Selected transition: {self.current_state} {current_symbol} -> {selected_transition.new_state} {selected_transition.write_symbol or 'no change'} {selected_transition.direction}"
        )
        ...
        transition_taken = {
            ...
            "write_symbol": (
                selected_transition.write_symbol
        ...
        if selected_transition.write_symbol:
            ...
            logger.debug(
                f"Wrote symbol: {selected_transition.write_symbol} at position {self.head}"
            )
        ...
        if selected_transition.direction == "r":
            ...
            if self.machine_type == "left-bounded":
                if self.head > 0:
                    ...
                    logger.debug(
                        f"Attempted to move left at position {self.head}; staying in place (Left-Bounded)"
                    )
            else:
                if self.head > 0:
                    self.head -= 1
                    ...
                    self.head = 0
                    logger.debug(
                        f"Extended tape left and moved head to position {self.head}"
                    )
        ...
        self.steps += 1
        logger.debug(
            f"Transition to new state: {self.current_state}, Total steps: {self.steps}"
        )
