...
const TapeDisplay: React.FC<TapeDisplayProps> = ({
  ...
  return (
    <div className="tape-display bg-white dark:bg-gray-700 p-4 rounded-lg shadow">
      ...
      <div className="flex items-center justify-start space-x-2 overflow-x-auto">
        {machineType === MachineType.LEFT_BOUNDED && (
          <div
            className="left-boundary w-8 h-8 flex items-center justify-center border border-red-500 bg-red-100 dark:bg-red-600 rounded"
        ...
        {tapeArray.map((symbol: string, index: number) => (
          <div
            key={index}
            className={`tape-symbol w-8 h-8 flex items-center justify-center border ${
              index === head
